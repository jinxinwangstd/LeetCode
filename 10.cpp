    ...
    bool isMatch(string s, string p) {
        ...
        for (int i = s.size(); i >= 0; --i) {
            for (int j = p.size() - 1; j >= 0; --j) {
                ...
                // up recursion
                if (j + 1 < p.size() && p[j + 1] == '*')    // wildcard is valid only with its preceding element, so we must process the wildcard in advance, when we process the preceding element
